...
contract FieldArithmeticTest is Test {
    ...
    function setUp() public {
        ...
        vectorsJson = vm.readFile(string.concat(TESTDATA, "field_vectors.json"));
    }
    ...
    function testKoalaBearBaseFieldVectors() external view {
        for (uint256 i = 0; _vectorExists("base", i); ++i) {
            ...
            assertEq(harness.baseAdd(a, b), vectorsJson.readUint(string.concat(prefix, ".add")));
            assertEq(harness.baseSub(a, b), vectorsJson.readUint(string.concat(prefix, ".sub")));
            assertEq(harness.baseMul(a, b), vectorsJson.readUint(string.concat(prefix, ".mul")));
            assertEq(harness.baseInv(a), vectorsJson.readUint(string.concat(prefix, ".inv")));
        }
    }
    ...
    function testKoalaBearExt4Vectors() external view {
        for (uint256 i = 0; _vectorExists("quartic", i); ++i) {
            ...
            uint256[] memory aCoeffs = vectorsJson.readUintArray(string.concat(prefix, ".a"));
            uint256[] memory bCoeffs = vectorsJson.readUintArray(string.concat(prefix, ".b"));

            uint256 packedA = uint256(vectorsJson.readBytes32(string.concat(prefix, ".packed_a")));
            uint256 packedB = uint256(vectorsJson.readBytes32(string.concat(prefix, ".packed_b")));
            ...
            assertEq(
                ...
                uint256(vectorsJson.readBytes32(string.concat(prefix, ".packed_add")))
            );
            assertEq(
                harness.ext4Sub(packedA, packedB),
                uint256(vectorsJson.readBytes32(string.concat(prefix, ".packed_sub")))
            );
            assertEq(
                harness.ext4Mul(packedA, packedB),
                uint256(vectorsJson.readBytes32(string.concat(prefix, ".packed_mul")))
            );
            assertEq(
                harness.ext4Inv(packedA),
                uint256(vectorsJson.readBytes32(string.concat(prefix, ".packed_inv")))
            );
            assertEq(harness.ext4MulByW(packedA), harness.ext4Pack(_scaleCoeffs(aCoeffs, 3)));
        }
    }
    ...
    function testKoalaBearExt8Vectors() external view {
        for (uint256 i = 0; _vectorExists("octic", i); ++i) {
            ...
            assertEq(harness.ext8MulByW(packedA), harness.ext8Pack(_scaleCoeffs(aCoeffs, 3)));
        }
    }
    ...
    function testGasEvaluateHypercubeExt8() external view {
        ...
        for (uint256 i = 0; i < 16; ++i) {
            evals[i] =
                harness.ext8Pack(_coeffs8(i + 1, i + 2, i + 3, i + 4, i + 5, i + 6, i + 7, i + 8));
        }
    ...
    function _vectorExists(string memory family, uint256 index) internal view returns (bool) {
        return vectorsJson.keyExists(string.concat(_vectorPrefix(family, index), ".a"));
    }

    function _vectorPrefix(string memory family, uint256 index)
        internal
        pure
