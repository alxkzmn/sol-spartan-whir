...
contract WhirVerifierLir11Test is Test {
    ...
    function testRejectsBadCommitmentFixture() external {
        ...
        WhirStructs.WhirProof memory failure = abi.decode(
            vm.readFileBinary(
                string.concat(
                    TESTDATA, "quartic_whir_lir11_ff5_rsv3_failure_bad_commitment_proof.abi"
                )
            ),
    ...
    function testRejectsBadStirQueryFixture() external {
        ...
        WhirStructs.WhirProof memory failure = abi.decode(
            vm.readFileBinary(
                string.concat(
                    TESTDATA, "quartic_whir_lir11_ff5_rsv3_failure_bad_stir_query_proof.abi"
                )
            ),
            (WhirStructs.WhirProof)
        );
    ...
    function testRejectsBadOodFixture() external {
        ...
        WhirStructs.WhirProof memory failure = abi.decode(
            vm.readFileBinary(
                string.concat(
                    TESTDATA,
                    "quartic_whir_lir11_ff5_rsv3_failure_bad_ood_or_transcript_mismatch_proof.abi"
                )
            ),
            (WhirStructs.WhirProof)
        );
    ...
    function testRejectsTamperedInitialSumcheckData() external {
        ...
        proof.initialSumcheck.polynomialEvals[0] =
            _incrementPackedExt4(proof.initialSumcheck.polynomialEvals[0]);
    ...
    function testRejectsMalformedInitialSumcheckEvals() external {
        ...
        _assertRejectsMalformedPackedExt4AtInitialSumcheck(0, 2, 2 * 0x7f000001);
    ...
    function testRejectsFinalConstraintMismatch() external {
        ...
        statement.evaluations[0] = _incrementPackedExt4(statement.evaluations[0]);
    ...
    function testRejectsTamperedFinalMerklePath() external {
        ...
        proof.finalQueryBatch.values[0] = (proof.finalQueryBatch.values[0] + 1) % 0x7f000001;
    ...
    function _incrementPackedExt4(uint256 packed) internal pure returns (uint256) {
    ...
    function _assertRejectsMalformedPackedExt4AtInitialOod(
        ...
        proof.initialOodAnswers[index] = _withLane(proof.initialOodAnswers[index], lane, value);

        vm.expectPartialRevert(WhirVerifierUtils4.PackedExtensionElementOutOfRange.selector);
        verifier.verify(proof.initialCommitment, statement, proof);
    }

    function _assertRejectsMalformedLowBitsAtInitialOod(uint256 index) internal {
        (WhirStructs.WhirStatement memory statement, WhirStructs.WhirProof memory proof) =
            _loadSuccessFixture();

        proof.initialOodAnswers[index] = _withLowBits(proof.initialOodAnswers[index]);
    ...
    function _assertRejectsMalformedPackedExt4AtInitialSumcheck(
        ...
        proof.initialSumcheck.polynomialEvals[index] =
            _withLane(proof.initialSumcheck.polynomialEvals[index], lane, value);

        vm.expectPartialRevert(WhirVerifierUtils4.PackedExtensionElementOutOfRange.selector);
        verifier.verify(proof.initialCommitment, statement, proof);
    }

    function _assertRejectsMalformedLowBitsAtInitialSumcheck(uint256 index) internal {
        (WhirStructs.WhirStatement memory statement, WhirStructs.WhirProof memory proof) =
            _loadSuccessFixture();

        proof.initialSumcheck.polynomialEvals[index] =
            _withLowBits(proof.initialSumcheck.polynomialEvals[index]);
