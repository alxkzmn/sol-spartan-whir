...
contract TranscriptParityK22Jb100Test is Test {
    ...
    function testReplayTranscriptCheckpointParity() external view {
        ...
        uint256[8] memory proverCheckpoint = prover.sampleExt8Coeffs();
        uint256[8] memory verifierCheckpoint = verifier.sampleExt8Coeffs();
    ...
    function testReplaySpartanTranscriptContextJohnsonBound() external view {
        ...
        assertEq(fixture.soundnessAssumption, 1);
    ...
    function _loadTrace() internal view returns (TranscriptTrace memory) {
        bytes memory raw = vm.readFileBinary(
            string.concat(TESTDATA, "transcript_trace_octic_k22_jb100_lir6_ff4_rsv1.abi")
        );
    ...
    function _loadSpartanContextFixture() internal view returns (SpartanContextFixture memory) {
        bytes memory raw = vm.readFileBinary(
            string.concat(TESTDATA, "spartan_transcript_context_octic_k22_jb100_lir6_ff4_rsv1.abi")
        );
    ...
    function _assertCheckpoint(KeccakChallenger.State memory challenger, uint256[] memory expected)
        ...
        pure
    {
        uint256[8] memory checkpoint = challenger.sampleExt8Coeffs();
