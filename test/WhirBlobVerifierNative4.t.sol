...
import {WhirBlobCodec4} from "../src/whir/WhirBlobCodec4.sol";
import {WhirBlobVerifier4} from "../src/whir/WhirBlobVerifier4.sol";
import {WhirBlobVerifierNative4} from "../src/whir/WhirBlobVerifierNative4.sol";
...
contract WhirBlobVerifierNative4Test is Test {
    ...
    function testNativeBlobUsesLessGasThanWrapper() external view {
        ...
        bool nativeOk = nativeBlobVerifier.verify(
    ...
    function testRejectsBadCommitmentBlobNative() external {
        ...
        bytes memory blob = vm.readFileBinary(
            string.concat(TESTDATA, "quartic_whir_failure_bad_commitment.blob")
        );
    ...
    function testRejectsBadStirQueryBlobNative() external {
        (, WhirStructs.WhirProof memory success) = _loadSuccessFixture();
        bytes memory blob = vm.readFileBinary(
            string.concat(TESTDATA, "quartic_whir_failure_bad_stir_query.blob")
        );
    ...
    function _computeOffsets(
        WhirStructs.WhirProof memory proof
    ) internal pure returns (BlobOffsets memory offsets) {
    ...
    function _readU32(
        ...
        value =
            (uint8(blob[offset]) << 24) |
            (uint8(blob[offset + 1]) << 16) |
            (uint8(blob[offset + 2]) << 8) |
            uint8(blob[offset + 3]);
    }
