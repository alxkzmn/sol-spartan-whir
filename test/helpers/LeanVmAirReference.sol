...
import { LeanVmPoseidonAirConstants as C } from "../../src/leanvm/LeanVmPoseidonAirConstants.sol";
...
library LeanVmAirReference {
    ...
    function evalExecution(
        ...
        Evaluator memory e = _start(flat, shift, alphas, betaEq, 20, 2, 14);
        uint256[3] memory nf;
        uint256[3] memory nu;
        for (uint256 i; i < 3; ++i) {
    ...
    function evalExtension(
        ...
        Evaluator memory e = _start(flat, shift, alphas, betaEq, 29, 13, 35);
    ...
    function evalPoseidon(
        ...
        Evaluator memory e = _start(flat, shift, alphas, betaEq, 110, 0, 96);
        ...
        for (uint256 r; r < 20; ++r) {
            ...
            for (uint256 i; i < 16; ++i) {
                new0 = new0.add(
                    state[i].mulBase(_constant(constants, C.SPARSE_FIRST_ROW + r * 16 + i))
                );
            }
            state[0] = new0;
            for (uint256 i = 1; i < 16; ++i) {
                state[i] =
                    state[i].add(old0.mulBase(_constant(constants, C.SPARSE_V + r * 16 + i - 1)));
            }
        }
    ...
    function _start(
        ...
        uint256 shifts,
        uint256 constraints
    ) private pure returns (Evaluator memory e) {
        if (
        ...
        for (uint256 r; r < 2; ++r) {
            ...
            for (uint256 i; i < 16; ++i) {
                ...
                for (uint256 j; j < 16; ++j) {
                    sum = sum.add(sbox[j].mulBase(firstRow[(j + 16 - i) % 16]));
                }
                state[i] = sum;
            }
        }
        ...
        returns (uint256[16] memory result)
    {
        for (uint256 i; i < 16; ++i) {
            for (uint256 j; j < 16; ++j) {
                result[i] =
                    result[i].add(state[j].mulBase(_constant(constants, offset + i * 16 + j)));
            }
        }
    }
}
