...
contract FixtureDecodeTest is Test {
    ...
    function testDecodeQuarticWhirSuccessFixtures() external view {
        ...
        WhirStructs.WhirProof memory proof = abi.decode(proofRaw, (WhirStructs.WhirProof));
        WhirStructs.WhirStatement memory statement =
            abi.decode(statementRaw, (WhirStructs.WhirStatement));
    ...
    function testDecodeTamperedFailureProof() external view {
        ...
        bytes memory failureRaw = vm.readFileBinary(
            string.concat(TESTDATA, "quartic_whir_lir6_ff5_rsv1_failure_bad_commitment_proof.abi")
        );

        WhirStructs.WhirProof memory success = abi.decode(successRaw, (WhirStructs.WhirProof));
        WhirStructs.WhirProof memory failure = abi.decode(failureRaw, (WhirStructs.WhirProof));
    ...
    function testDecodeTamperedStirQueryFailureProof() external view {
        ...
        bytes memory failureRaw = vm.readFileBinary(
            string.concat(TESTDATA, "quartic_whir_lir6_ff5_rsv1_failure_bad_stir_query_proof.abi")
        );
        ...
        assertGt(failure.finalPoly.length, 0);
        assertTrue(_firstQueryBatchHash(success) != _firstQueryBatchHash(failure));
    }
    ...
    function testDecodeTamperedOodOrTranscriptMismatchFailureProof() external view {
        bytes memory successRaw = vm.readFileBinary(
            string.concat(TESTDATA, "quartic_whir_lir6_ff5_rsv1_success_proof.abi")
        );
        bytes memory failureRaw = vm.readFileBinary(
            string.concat(
                TESTDATA,
                "quartic_whir_lir6_ff5_rsv1_failure_bad_ood_or_transcript_mismatch_proof.abi"
            )
        );
    ...
    function testDecodeSpartanPlaceholderWithRealPcsProof() external view {
        bytes memory proofRaw = vm.readFileBinary(
            string.concat(TESTDATA, "spartan_placeholder_proof_quartic_lir6_ff5_rsv1.abi")
        );
        SpartanStructs.SpartanProof memory proof =
            abi.decode(proofRaw, (SpartanStructs.SpartanProof));
    ...
    function testDecodeSpartanPlaceholderInstance() external view {
        bytes memory instanceRaw =
            vm.readFileBinary(string.concat(TESTDATA, "spartan_placeholder_instance.abi"));
        SpartanStructs.SpartanInstance memory instance =
            abi.decode(instanceRaw, (SpartanStructs.SpartanInstance));
    ...
    function _firstQueryBatchHash(WhirStructs.WhirProof memory proof)
        internal
        pure
        returns (bytes32)
    {
