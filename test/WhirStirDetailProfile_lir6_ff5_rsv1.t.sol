...
contract WhirStirDetailHarness {
    ...
    function profileFinalStirSplit(
        ...
        WhirVerifierCore4.Constraint[] memory constraints = new WhirVerifierCore4.Constraint[](3);
        ...
        uint256 claimedEval = WhirVerifierCore4._combineConstraintEvals(0, constraints[0]);
        ...
        unchecked {
            for (uint256 roundIndex = 0; roundIndex < proof.rounds.length; ++roundIndex) {
                QuarticWhirFixedConfig.RoundConfig memory roundConfig =
                    QuarticWhirFixedConfig.roundConfig(roundIndex);
                WhirStructs.WhirRoundProof calldata roundProof = proof.rounds[roundIndex];

                WhirVerifierCore4.ParsedCommitment memory newCommitment =
                ...
                WhirVerifierCore4.SelectStatement memory stirStatement =
                ...
                claimedEval = WhirVerifierCore4._combineConstraintEvals(
                    claimedEval, constraints[constraintCount]
                );
                constraintCount += 1;

                uint256 nextFoldingFactor = roundIndex + 1 < QuarticWhirFixedConfig.ROUND_COUNT
                    ? QuarticWhirFixedConfig.roundConfig(roundIndex + 1).foldingFactor
    ...
    function _profileFrontierAndMerkleRaw(
        ...
        uint256[] memory frontierIndices = new uint256[](indices.length);
        bytes32[] memory frontierHashes = new bytes32[](indices.length);
        g = gasleft();
        unchecked {
            for (uint256 i = 0; i < indices.length; ++i) {
                frontierIndices[i] = indices[i];
                frontierHashes[i] = expectedKind == 0
        ...
        g = gasleft();
        bytes32 computedRoot = _computeRootFromFrontierProfile(
            frontierIndices, frontierHashes, depth, queryBatch.decommitments, effectiveDigestBytes
        );
        ...
        bd.total = startGas - gasleft();
        uint256 measured = bd.sampleQueries + bd.frontierInit + bd.merkleLoopOnly;
        bd.overhead = bd.total - measured;
    }

    function _computeRootFromFrontierProfile(
        uint256[] memory frontierIndices,
        bytes32[] memory frontierHashes,
        uint256 depth,
        bytes32[] calldata decommitments,
        uint256 effectiveDigestBytes
    ) internal pure returns (bytes32) {
        require(frontierIndices.length != 0, "EMPTY_INDICES");
        require(frontierIndices.length == frontierHashes.length, "LEN_MISMATCH");

        unchecked {
            for (uint256 i = 1; i < frontierIndices.length; ++i) {
                require(frontierIndices[i - 1] < frontierIndices[i], "UNSORTED");
            }
        }

        uint256 frontierLen = frontierIndices.length;
...
contract WhirStirDetailProfileTest is Test {
    ...
    function testProfileStirFrontierBreakdown() external view {
        ...
        ) = harness.profileStirFrontierBreakdowns(proof.initialCommitment, statement, proof);
    ...
    function testProfileFinalStirSplit() external view {
        ...
        WhirStirDetailHarness.FinalStirSplit memory split =
            harness.profileFinalStirSplit(proof.initialCommitment, statement, proof);
