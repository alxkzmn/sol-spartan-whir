...
import {
    KoalaBearEqBaselineHarness,
    KoalaBearEqPeelHarness,
    KoalaBearEqFusedHarness,
    KoalaBearEqCombinedHarness,
    KoalaBearEqPackedMulHarness,
    KoalaBearEqPackedSquareHarness,
    KoalaBearEqPackedEqHarness,
    KoalaBearEqNoFinalSquaresHarness,
    KoalaBearEqPackedMulNoFinalHarness,
    BabyBearEqBaselineHarness,
    BabyBearEqPeelHarness,
    BabyBearEqFusedHarness,
    BabyBearEqCombinedHarness,
    BabyBearEqPackedMulHarness,
    BabyBearEqPackedSquareHarness,
    BabyBearEqPackedEqHarness,
    BabyBearEqNoFinalSquaresHarness,
    BabyBearEqPackedMulNoFinalHarness
...
contract BabyBearEqBenchmarkTest is Test {
    IEqHarness[18] h;

    function setUp() external {
        h[0] = IEqHarness(address(new KoalaBearEqBaselineHarness()));
        h[1] = IEqHarness(address(new KoalaBearEqPeelHarness()));
        h[2] = IEqHarness(address(new KoalaBearEqFusedHarness()));
        h[3] = IEqHarness(address(new KoalaBearEqCombinedHarness()));
        h[4] = IEqHarness(address(new KoalaBearEqPackedMulHarness()));
        h[5] = IEqHarness(address(new KoalaBearEqPackedSquareHarness()));
        h[6] = IEqHarness(address(new KoalaBearEqPackedEqHarness()));
        h[7] = IEqHarness(address(new KoalaBearEqNoFinalSquaresHarness()));
        h[8] = IEqHarness(address(new KoalaBearEqPackedMulNoFinalHarness()));
        h[9] = IEqHarness(address(new BabyBearEqBaselineHarness()));
        h[10] = IEqHarness(address(new BabyBearEqPeelHarness()));
        h[11] = IEqHarness(address(new BabyBearEqFusedHarness()));
        h[12] = IEqHarness(address(new BabyBearEqCombinedHarness()));
        h[13] = IEqHarness(address(new BabyBearEqPackedMulHarness()));
        h[14] = IEqHarness(address(new BabyBearEqPackedSquareHarness()));
        h[15] = IEqHarness(address(new BabyBearEqPackedEqHarness()));
        h[16] = IEqHarness(address(new BabyBearEqNoFinalSquaresHarness()));
        h[17] = IEqHarness(address(new BabyBearEqPackedMulNoFinalHarness()));
    }
    ...
    function testFuzzSquareAndEq(uint256[5] memory a, uint256[5] memory b) external view {
        for (uint256 i; i < 18; ++i) {
            uint256 p = i < 9 ? 0x7f000001 : 0x78000001;
            uint256 f = i < 9 ? 0 : 1;
    ...
    function _checkPreparation(uint256 seed) private view {
        ...
        for (uint256 f; f < 2; ++f) {
            ...
            for (uint256 j = f == 0 ? 0 : 9; j < (f == 0 ? 9 : 18); ++j) {
    ...
    function testBenchmarkPreparation() external {
        ...
        string[18] memory names = [
            ...
            "KoalaBearEqPackedSquare",
            "KoalaBearEqPackedEq",
            "KoalaBearEqNoFinalSquares",
            "KoalaBearEqPackedMulNoFinal",
            "BabyBearEqBaseline",
            "BabyBearEqPeel",
            "BabyBearEqFused",
            "BabyBearEqCombined",
            "BabyBearEqPackedMul",
            "BabyBearEqPackedSquare",
            "BabyBearEqPackedEq",
            "BabyBearEqNoFinalSquares",
            "BabyBearEqPackedMulNoFinal"
        ];
        for (uint256 i; i < 18; ++i) {
    ...
    function testProfileBestEquality() external view {
        (bytes memory statement, uint256[4] memory ood, uint256[] memory point) =
            _inputs(20_260_905);
        (, uint256[5] memory result) = h[13].prepare(statement, ood, point);
