...
contract TranscriptParityTest is Test {
    ...
    function testObserveBaseMatchesRawLittleEndianBytes() external pure {
        ...
        assertEq(KeccakChallenger.debugInputHash(lhs), KeccakChallenger.debugInputHash(rhs));
    ...
    function testObserveHashU64DigestMatchesLittleEndianWords() external pure {
        ...
        rhs.observeBytes(hex"07060504030201000f0e0d0c0b0a090817161514131211101f1e1d1c1b1a1918");
    ...
    function testReplayVerifierWhirTranscriptSemantically() external view {
        ...
        cursor = _replayOodSemantically(
            challenger, trace.verifierEvents, cursor, proof.initialOodAnswers
        );

        cursor = _consumeSampleEventsUntilObserve(challenger, trace.verifierEvents, cursor);

        cursor = _replaySumcheckSemantically(
            challenger, trace.verifierEvents, cursor, proof.initialSumcheck
        );
        ...
        unchecked {
            for (uint256 i = 0; i < proof.rounds.length; ++i) {
                ...
                cursor = _replayOodSemantically(
                    challenger, trace.verifierEvents, cursor, round.oodAnswers
                );

                // STIR pow witness: checkWitness does observeBase(witness) + sampleBits
                cursor = _replayPowWitnessIfPresent(challenger, trace.verifierEvents, cursor);

                cursor = _consumeSampleEventsUntilObserve(challenger, trace.verifierEvents, cursor);

                cursor = _replaySumcheckSemantically(
                    challenger, trace.verifierEvents, cursor, round.sumcheck
                );
            }
        }
        ...
        if (proof.finalSumcheckPresent) {
            cursor = _replaySumcheckSemantically(
                challenger, trace.verifierEvents, cursor, proof.finalSumcheck
            );
        }
    ...
    function testReplaySpartanTranscriptContext() external view {
        ...
        SpartanTranscript.DomainSeparator memory domainSeparator = SpartanTranscript.DomainSeparator({
        ...
        bytes memory preimage = SpartanTranscript.domainSeparatorPreimage(domainSeparator);
        ...
        (challenger, observedDigest) = SpartanTranscript.observeSpartanContext(
            challenger, domainSeparator, fixture.publicInputs
        );
    ...
    function _loadTrace() internal view returns (TranscriptTrace memory) {
        bytes memory raw = vm.readFileBinary(
            string.concat(TESTDATA, "transcript_trace_quartic_lir6_ff5_rsv1.abi")
        );
        return abi.decode(raw, (TranscriptTrace));
    }

    function _loadWhirProof() internal view returns (WhirStructs.WhirProof memory) {
        bytes memory raw = vm.readFileBinary(
    ...
    function _loadSpartanContextFixture() internal view returns (SpartanContextFixture memory) {
        bytes memory raw = vm.readFileBinary(
            string.concat(TESTDATA, "spartan_transcript_context_quartic_lir6_ff5_rsv1.abi")
        );
    ...
    function _replayOodSemantically(
        ...
        unchecked {
            for (uint256 i = 0; i < oodAnswers.length; ++i) {
                cursor = _consumeSampleEventsUntilObserve(challenger, events, cursor);
        ...
        unchecked {
            for (uint256 i = 0; i < numRounds; ++i) {
                challenger.observeSumcheckRoundPolyExt4(
                    sumcheck.polynomialEvals[2 * i], sumcheck.polynomialEvals[2 * i + 1]
                );
                cursor += 8;

                // Sumcheck pow witness (checkWitness = observeBase + sampleBits)
                if (i < sumcheck.powWitnesses.length) {
                    cursor = _replayPowWitnessIfPresent(challenger, events, cursor);
                }
    ...
    function _consumeSampleEventsUntilObserve(
        ...
        while (cursor < events.length && events[cursor].op != OP_OBSERVE_BYTES) {
            if (events[cursor].op == OP_SAMPLE_BASE) {
                ...
                cursor = _assertNextSampleBits(challenger, events, cursor, events[cursor].arg0);
            } else if (events[cursor].op == OP_GRIND) {
                assertTrue(challenger.checkWitness(events[cursor].arg0, events[cursor].arg1));
                unchecked {
                    ++cursor;
                }
            } else {
    ...
    function _assertSameState(KeccakChallenger.State memory lhs, KeccakChallenger.State memory rhs)
        internal
        pure
    {
        assertEq(KeccakChallenger.debugInputHash(lhs), KeccakChallenger.debugInputHash(rhs));
