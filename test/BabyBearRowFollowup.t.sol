...
import {
    KoalaBearRowFactoredFollowupHarness,
    KoalaBearRowCombinedFollowupHarness,
    KoalaBearRowStreamFactoredFollowupHarness,
    KoalaBearRowStreamCombinedFollowupHarness,
    BabyBearRowStreamFactoredFollowupHarness,
    BabyBearRowStreamCombinedFollowupHarness
} from "./helpers/BabyBearRowFollowup.sol";

interface IRowFollowupHarness {
...
contract BabyBearRowFollowupTest is Test {
    IRowFollowupHarness[8] h;

    function setUp() external {
        h[0] = IRowFollowupHarness(address(new KoalaBearRowKroneckerHarness()));
        h[1] = IRowFollowupHarness(address(new KoalaBearRowFactoredFollowupHarness()));
        h[2] = IRowFollowupHarness(address(new KoalaBearRowCombinedFollowupHarness()));
        h[3] = IRowFollowupHarness(address(new KoalaBearRowStreamFactoredFollowupHarness()));
        h[4] = IRowFollowupHarness(address(new KoalaBearRowStreamCombinedFollowupHarness()));
        h[5] = IRowFollowupHarness(address(new BabyBearRowKroneckerStreamHarness()));
        h[6] = IRowFollowupHarness(address(new BabyBearRowStreamFactoredFollowupHarness()));
        h[7] = IRowFollowupHarness(address(new BabyBearRowStreamCombinedFollowupHarness()));
    }
        ...
        view
    {
        ...
        for (uint256 i = f == 0 ? 0 : 5; i < (f == 0 ? 5 : 8); ++i) {
    ...
    function testProfileSelectedBabyRows() external view {
        ...
        (,, uint256 value,) = h[7].batch(_blob(bytes32(uint256(73)), 31, 0x78000001), point, 31);
        assertTrue(value != 0);
    }
}
