#!/usr/bin/env python3
"""Independent bounds and differential checks for fused 17-term quintic dot products.
...
per element, including claim times challenge as the seventeenth term, followed by delayed coefficient reconstruction.
"""
...
def check_bounds(p, baby):
    ...
    final_bound = 17 * reduction_weight * (p - 1)**2
    assert final_bound < 1 << (70 if baby else 69)
    return {
        "prime": p,
...
def run_field(p, baby, random_cases, seed):
    ...
    for x in boundaries:
        for y in boundaries:
            check([[x]*5]*17, [[y]*5]*17)
    for count in (1, 2, 4, 8, 15, 16, 17):
        check([[p-1]*5]*count, [[p-1]*5]*count)
        check([[0, p-1, 0, p-1, 0]]*count, [[p-1, 0, p-1, 0, p-1]]*count)
    rng = random.Random(seed)
    for _ in range(random_cases):
        check([[rng.randrange(p) for _ in range(5)] for _ in range(17)],
              [[rng.randrange(p) for _ in range(5)] for _ in range(17)])
    return {"field": "BabyBear" if baby else "KoalaBear", "cases": cases,
            "random_17_term_cases": random_cases, "bounds": bounds}
