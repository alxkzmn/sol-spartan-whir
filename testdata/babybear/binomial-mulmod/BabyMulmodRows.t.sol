...
import {BabyBearRowStreamCombinedFollowupHarness,BabyBearRowMulmodHarness,BabyBearRowMulmodUMaskHarness} from "./helpers/Rows.sol";
...
contract BabyBearMulmodRowsTest is Test {
    IRowFollowupHarness[3] h;

    function setUp() external {h[2]=IRowFollowupHarness(address(new BabyBearRowMulmodUMaskHarness()));h[0]=IRowFollowupHarness(address(new BabyBearRowStreamCombinedFollowupHarness()));h[1]=IRowFollowupHarness(address(new BabyBearRowMulmodHarness()));}
    ...
    function testBenchmarkFollowupRows() external {
        string[3] memory names=["BabyCombined","BabyMulmod","BabyMulmodUMask"];
        uint256[4] memory point;
        for (uint256 j; j < 4; ++j) {
...
function testFuzzIndependentDot(bytes32 seed) external view {uint256[16] memory a;uint256[16] memory b;uint256 expected;for(uint256 i;i<16;++i){a[i]=_packed(seed,i,0x78000001);b[i]=_packed(seed,i+16,0x78000001);expected=Ref.add(expected,Ref.mul(a[i],b[i],0x78000001,1),0x78000001);}for(uint256 j;j<3;++j){(,uint256 actual)=h[j].dot(a,b);assertEq(actual,expected);}}
function testMaximalDotAndBasis() external{uint256 Q=0x78000001;uint256[16] memory a;uint256[16] memory b;uint256 max=Ref.pack([Q-1,Q-1,Q-1,Q-1,Q-1]);for(uint256 i;i<16;++i){a[i]=max;b[i]=max;}uint256 expected=Ref.scale(Ref.mul(max,max,Q,1),16,Q);for(uint256 j;j<3;++j){(uint256 used,uint256 actual)=h[j].dot(a,b);assertEq(actual,expected);emit log_named_uint(j==0?"BabyCombined.dot":"BabyMulmod.dot",used);}for(uint256 i;i<5;++i)for(uint256 j;j<5;++j){for(uint256 z;z<16;++z){a[z]=(Q-1)<<(224-32*i);b[z]=(Q-1)<<(224-32*j);}expected=Ref.scale(Ref.mul(a[0],b[0],Q,1),16,Q);(,uint256 actual)=h[2].dot(a,b);assertEq(actual,expected);}}
}
