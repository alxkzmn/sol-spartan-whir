...
contract WhirBlobNativeTxBenchmarkScript is Script {
    ...
    function run() external returns (address nativeVerifierAddress) {
        ...
        bytes memory verifyCalldata = abi.encodeCall(
            WhirBlobVerifierNative4.verify,
        ...
        console2.log(
            "native blob verify calldata bytes",
            verifyCalldata.length
        );
        console2.log("native blob verify calldata zero bytes", zeroBytes);
        console2.log(
            "native blob verify calldata non-zero bytes",
            nonZeroBytes
        );
        console2.log(
            "native blob verify calldata intrinsic gas",
            uint256(21_000)
        );
        ...
        (bool ok, bytes memory ret) = nativeVerifierAddress.call(
            verifyCalldata
        );
