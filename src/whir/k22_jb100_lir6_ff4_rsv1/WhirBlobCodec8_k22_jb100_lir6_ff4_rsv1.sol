...
library WhirBlobCodec8 {
    ...
    uint8 internal constant EXTENSION_DEGREE = 8;
    ...
    uint256 internal constant ROUND0_NUM_QUERIES = 24;
    uint256 internal constant ROUND1_NUM_QUERIES = 16;
    uint256 internal constant ROUND2_NUM_QUERIES = 12;
        ...
        returns (WhirStructs.WhirStatement memory statement, WhirStructs.WhirProof memory proof)
    {
        ...
        for (uint256 i = 0; i < FINAL_POLY_LEN; ++i) {
            (proof.finalPoly[i], offset) = readExt8(blob, offset);
        }
        ...
        for (uint256 i = 0; i < FINAL_NUM_QUERIES * ROW_LEN; ++i) {
            (proof.finalQueryBatch.values[i], offset) = readExt8(blob, offset);
        }
        ...
        for (uint256 i = 0; i < FINAL_SUMCHECK_EVALS; ++i) {
            (proof.finalSumcheck.polynomialEvals[i], offset) = readExt8Le(blob, offset);
        }
    ...
    function _decodeRound(
        ...
        (round.oodAnswers[0], offset) = readExt8Le(blob, offset);
        ...
        for (uint256 i = 0; i < numQueries * ROW_LEN; ++i) {
            if (isBase) {
                ...
                (round.queryBatch.values[i], offset) = readExt8(blob, offset);
            }
        }
        ...
        for (uint256 i = 0; i < ROUND_SUMCHECK_EVALS; ++i) {
            (round.sumcheck.polynomialEvals[i], offset) = readExt8Le(blob, offset);
        }
    ...
    function expectedLength(
        ...
        return HEADER_BYTES + STATEMENT_POINT_ARITY * 32 + STATEMENT_EVALUATIONS * 32
            + EFFECTIVE_DIGEST_BYTES + INITIAL_OOD_SAMPLES * 32 + INITIAL_SUMCHECK_EVALS * 32
            + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 32 + 4 + ROUND0_NUM_QUERIES * ROW_LEN * 4
            + round0DecommLen * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 32
            + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 32 + 4 + ROUND1_NUM_QUERIES * ROW_LEN
            * 32 + round1DecommLen * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 32
            + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 32 + 4 + ROUND2_NUM_QUERIES * ROW_LEN
            * 32 + round2DecommLen * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 32
            + FINAL_POLY_LEN * 32 + 4 + FINAL_NUM_QUERIES * ROW_LEN * 32 + finalDecommLen
            * EFFECTIVE_DIGEST_BYTES + FINAL_SUMCHECK_EVALS * 32;
    }
    ...
    function readExt8(bytes calldata blob, uint256 offset)
        internal
        pure
        returns (uint256 value, uint256 next)
    {
        assembly ("memory-safe") {
            value := calldataload(add(blob.offset, offset))
        }
        next = offset + 32;
    }
    ...
    function readExt8Le(bytes calldata blob, uint256 offset)
        internal
        pure
        returns (uint256 value, uint256 next)
    {
        ...
        value = (_bswap32(uint32(word >> 224)) << 224) | (_bswap32(uint32(word >> 192)) << 192)
            ...
            | (_bswap32(uint32(word >> 96)) << 96) | (_bswap32(uint32(word >> 64)) << 64)
            | (_bswap32(uint32(word >> 32)) << 32) | _bswap32(uint32(word));
        next = offset + 32;
    }
