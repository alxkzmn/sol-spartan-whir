...
library WhirBlobCodec5 {
    ...
    uint8 internal constant ROUND_COUNT = 4;
    uint8 internal constant FLAGS = 0x03;

    uint256 internal constant HEADER_BYTES = 20;
    ...
    uint256 internal constant ROUND_SUMCHECK_EVALS = 6;
    uint256 internal constant FINAL_SUMCHECK_EVALS = 12;
    uint256 internal constant INITIAL_SUMCHECK_POW_WITNESSES = 4;
    uint256 internal constant ROUND_SUMCHECK_POW_WITNESSES = 3;
    uint256 internal constant ROUND0_NUM_QUERIES = 39;
    uint256 internal constant ROUND1_NUM_QUERIES = 39;
    uint256 internal constant ROUND2_NUM_QUERIES = 26;
    uint256 internal constant ROUND3_NUM_QUERIES = 19;
    uint256 internal constant FINAL_NUM_QUERIES = 16;
    uint256 internal constant ROW_LEN = 16;
    uint256 internal constant REST_ROW_LEN = 8;
        ...
        returns (WhirStructs.WhirStatement memory statement, WhirStructs.WhirProof memory proof)
    {
        ...
        offset =
            _decodeRound(
                blob, offset, round0DecommLen, proof.rounds[0], true, ROUND0_NUM_QUERIES, ROW_LEN
            );
        offset =
            _decodeRound(
                blob,
        ...
        proof.finalQueryBatch.rowLen = REST_ROW_LEN;
        proof.finalQueryBatch.values = new uint256[](FINAL_NUM_QUERIES * REST_ROW_LEN);
        for (uint256 i = 0; i < FINAL_NUM_QUERIES * REST_ROW_LEN; ++i) {
    ...
    function _decodeRound(
        ...
        round.queryBatch.rowLen = rowLen;
        round.queryBatch.values = new uint256[](numQueries * rowLen);
        for (uint256 i = 0; i < numQueries * rowLen; ++i) {
            if (isBase) {
        ...
        )
    {
        ...
        assembly ("memory-safe") {
            ...
            round3DecommLen := shr(240, calldataload(add(base, 16)))
            finalDecommLen := shr(240, calldataload(add(base, 18)))
        }
    ...
    function expectedLength(
        ...
        return HEADER_BYTES + STATEMENT_POINT_ARITY * 20 + STATEMENT_EVALUATIONS * 20
            ...
            + ROUND_OOD_SAMPLES * 20 + 4 + ROUND1_NUM_QUERIES * REST_ROW_LEN * 20 + round1DecommLen
            * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 20 + ROUND_SUMCHECK_POW_WITNESSES * 4
            + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 20 + 4 + ROUND2_NUM_QUERIES * REST_ROW_LEN
            * 20 + round2DecommLen * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 20
            + ROUND_SUMCHECK_POW_WITNESSES * 4 + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 20
            + 4 + ROUND3_NUM_QUERIES * REST_ROW_LEN * 20 + round3DecommLen
            * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 20
            + ROUND_SUMCHECK_POW_WITNESSES * 4 + FINAL_POLY_LEN * 20 + 4 + FINAL_NUM_QUERIES
            * REST_ROW_LEN * 20 + finalDecommLen * EFFECTIVE_DIGEST_BYTES + FINAL_SUMCHECK_EVALS * 20;
    }
