...
library WhirVerifierCore5 {
        ...
        unchecked {
            ...
            for (uint256 i = numQueries; i > 0; --i) {
                ...
                if (expectedKind == 0) {
                    ...
                    evalValue = WhirVerifierUtils5.evaluateBaseRowDim5BlobAfterHash(
                        blob, rowOffset, allRandomness, randomnessOffset
                    );
                } else {
    ...
    function _verifyRoundStirAndCombineConstraintBlobBase32(
        KeccakChallenger.State memory challenger,
        bytes32 expectedRoot,
        bytes calldata blob,
        uint256 valuesOffset,
        ...
        )
    {
        ...
        uint256[] memory indices =
            WhirVerifierUtils5.sampleStirQueries(challenger, 67_108_864, 5, 39);
        if (indices.length != 39) {
            revert QueryBatchCountMismatch(39, indices.length);
        }

        uint256 decommOffset = valuesOffset + 39 * 32 * 4;
        nextOffset = decommOffset + decommLen * 20;
    ...
    function _verifyFinalStirChallengesBlobFixed(
        ...
        uint256[] memory indices =
            WhirVerifierUtils5.sampleStirQueries(challenger, 1_048_576, 4, 14);
    ...
    function _parseFixedCommitment17x1Blob(
        KeccakChallenger.State memory challenger,
        bytes calldata blob,
        uint256 offset
    )
    ...
    function _parseFixedCommitment13x1Blob(
        KeccakChallenger.State memory challenger,
        bytes calldata blob,
        uint256 offset
    )
