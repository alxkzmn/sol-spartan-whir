...
contract WhirVerifier4 {
    ...
    function _combineInitialConstraintEvals(
        ...
        if (
            initialOodAnswers.length == QuarticWhirFixedConfig.COMMITMENT_OOD_SAMPLES
    ...
    function _evaluateInitialConstraint(
        ...
        if (statement.points.length == 1) {
            if (statement.points[0].length != QuarticWhirFixedConfig.NUM_VARIABLES) {
                revert WhirVerifierCore4.StatementPointArityMismatch(
                    0, QuarticWhirFixedConfig.NUM_VARIABLES, statement.points[0].length
                );
            }
        ...
        unchecked {
            for (uint256 i = QuarticWhirFixedConfig.COMMITMENT_OOD_SAMPLES; i > 0; --i) {
            ...
            for (uint256 i = statement.points.length; i > 0; --i) {
                ...
                uint256 weight = WhirVerifierCore4._eqPolyEvalAtCalldata(
                    statement.points[i - 1], allRandomness, 0, QuarticWhirFixedConfig.NUM_VARIABLES
                );
                assembly ("memory-safe") {
                    let M := 0x7f000001
                    let m := 0xffffffff
