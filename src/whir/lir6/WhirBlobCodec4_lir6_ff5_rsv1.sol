...
library WhirBlobCodec4 {
        ...
        returns (WhirStructs.WhirStatement memory statement, WhirStructs.WhirProof memory proof)
    {
        ...
        for (uint256 i = 0; i < INITIAL_SUMCHECK_EVALS; ++i) {
            (proof.initialSumcheck.polynomialEvals[i], offset) = readExt4Le(blob, offset);
        }
        ...
        for (uint256 i = 0; i < FINAL_SUMCHECK_EVALS; ++i) {
            (proof.finalSumcheck.polynomialEvals[i], offset) = readExt4Le(blob, offset);
        }
    ...
    function _decodeRound0(
        ...
        for (uint256 i = 0; i < ROUND_SUMCHECK_EVALS; ++i) {
            (round.sumcheck.polynomialEvals[i], offset) = readExt4Le(blob, offset);
        }
    ...
    function _decodeRound1(
        ...
        round.sumcheck.powWitnesses = new uint256[](ROUND1_SUMCHECK_POW_WITNESSES);
    ...
    function validateHeader(bytes calldata blob)
        internal
        pure
        returns (uint256 round0DecommLen, uint256 round1DecommLen, uint256 finalDecommLen)
    {
    ...
    function expectedLength(
        ...
        return HEADER_BYTES + STATEMENT_POINT_ARITY * 16 + STATEMENT_EVALUATIONS * 16
            + EFFECTIVE_DIGEST_BYTES + INITIAL_OOD_SAMPLES * 16 + INITIAL_SUMCHECK_EVALS * 16
            + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 16 + 4 + ROUND0_NUM_QUERIES * ROW_LEN * 4
            + round0DecommLen * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 16
            + EFFECTIVE_DIGEST_BYTES + ROUND_OOD_SAMPLES * 16 + 4 + ROUND1_NUM_QUERIES * ROW_LEN
            * 16 + round1DecommLen * EFFECTIVE_DIGEST_BYTES + ROUND_SUMCHECK_EVALS * 16
            + ROUND1_SUMCHECK_POW_WITNESSES * 4 + FINAL_POLY_LEN * 16 + 4 + FINAL_NUM_QUERIES
            * ROW_LEN * 16 + finalDecommLen * EFFECTIVE_DIGEST_BYTES + FINAL_SUMCHECK_EVALS * 16;
    }
    ...
    function readExt4(bytes calldata blob, uint256 offset)
        internal
        pure
    ...
    function readExt4Le(bytes calldata blob, uint256 offset)
        internal
        pure
        returns (uint256 value, uint256 next)
    {
        ...
        value = (_bswap32(c0) << 224) | (_bswap32(c1) << 192) | (_bswap32(c2) << 160)
            | (_bswap32(c3) << 128);
        next = offset + 16;
    }
